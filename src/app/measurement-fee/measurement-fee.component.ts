...
export class MeasurementFeeComponent implements OnInit {
  ...
  onFormSubmit() {
    ...
    if (this.fromM2 == null || this.fromM2 == undefined) {
      if (
        this.form.get('from_M2')?.value != null ||
        this.form.get('from_M2')?.value != undefined
      ) {
        this._empService.measurementfeeadd(this.form.value).subscribe(
          (res) => {
            this.measurementfeeList();
      ...
      this._empService.measurementfeeupdate(this.form.value).subscribe(
        (res) => {
          this.measurementfeeList();
          this.messageService.add({
            severity: 'success',
