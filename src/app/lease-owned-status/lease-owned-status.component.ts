...
@Component({
  ...
  templateUrl: './lease-owned-status.component.html',
  styleUrls: ['./lease-owned-status.component.css'],
  providers: [MessageService]

})
export class LeaseOwnedStatusComponent implements OnInit {
...
onFormSubmit() {
    ...
    if(this.form.get('status')?.value!=null||this.form.get('status')?.value!=undefined){
 ...
 //  })
  ...
  leaseownedstatuslist() {
    this._empService.leaseownedstatuslist().subscribe({
      next: (res) => {
        this.lease=res.procLease_Owned_Status_Lookups
        console.log('lease',res.procLease_Owned_Status_Lookups);
      },
      error: console.log,
    });
  }
  ...
  leaseownedstatusdelete(id: number) {
