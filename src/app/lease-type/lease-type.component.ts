...
@Component({
  selector: 'app-lease-type',
  templateUrl: './lease-type.component.html',
  styleUrls: ['./lease-type.component.css'],
  providers: [MessageService]

})
export class LeaseTypeComponent implements OnInit {
...
showSuccessMessage() {
  ...
  openEditForm(data: any) {
    ...
    this.form.patchValue(
      {
        lease_Type_ID:data.lease_Type_ID,
  ...
  onFormSubmit() {
    ...
    if(this.form.get('lease_Type')?.value!=null||this.form.get('lease_Type')?.value!=undefined){
 ...
 //  })
  ...
  leasetypeupdatelist() {
    this._empService.leasetypeupdatelist().subscribe({
      next: (res) => {
        this.LeaseType=res.procLease_Type_Lookups
        console.log('LeaseType',res.procLease_Type_Lookups);
